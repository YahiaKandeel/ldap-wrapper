...
class LDAP():
    ...
    def search(self, filter, attributes=None):
        ...
        self.pageControl = SimplePagedResultsControl(
        ...
        msgid = self.ldapClient.search_ext(
            self.base, ldap.SCOPE_SUBTREE, searchFilter, attributes, serverctrls=[self.pageControl])
    ...
    def dump_all(self):
        ...
        for cn in users:
            if cn not in self.data['computers'] and cn not in self.data['groups']:
                self.data['users'][cn] = users[cn]

        return self.data

    # search only users
    def dump_users(self):
        return self.search(filter='(objectclass=user)')
    # search only groups

    def dump_groups(self):
        return self.search(filter='(objectclass=group)')
    # Search by SAM

    def search_by_sam(self, sAMAccountName):
        return self.search(filter='(sAMAccountName=%s)' % sAMAccountName)
    # Search by cn

    def search_by_name(self, cn):
        return self.search(filter='(CN=%s)' % cn)
